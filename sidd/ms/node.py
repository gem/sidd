...
class StatisticNode (object):
    ...
    ###########################
    @logAPICall
    def leaves(self, taxonomy, with_modifier=True, parent_nodes=[], parent_weight = 1.0):
        branch_nodes = {"":[]}
        branch_weights = {"":1.0}
                ...
                for _mod_val, _mod_weight in mod.iteritems():   # loop through modifiers.values
                    ...
                    for branch, value in branch_nodes.iteritems():  # loop through existing branches
                        ...
                        if ( _mod_val is not None ):    # case 1
                            ...
                            branch_nodes[_branch_key].append(_mod_val) 
                            branch_weights[_branch_key] = value * _mod_weight
                        else:                           # case 2                            
                            branch_weights[branch] = value * _mod_weight
        ...
        for _branch_key, _nodes in branch_nodes.iteritems():
            ...
            if (self.is_leaf):
                parent_str = [str(n) for n in parent_nodes]
                parent_str.append(_branch_key)
                leaf_value = "/".join(parent_str) #taxonomy.to_string(parent_nodes)
                yield leaf_value, parent_weight * self.weight / 100.0 * _weight, self
            
            for _child in self.children:
                #print '\t','child leaf', _child.value                
                for _l in _child.leaves(taxonomy, with_modifier, parent_nodes, parent_weight * self.weight / 100.0 * _weight):
                    yield _l   
            # remove nodes
            parent_nodes.pop()
    ...
    def leaves2(self, separator, with_modifier=True, parent="", parent_weight = 1.0):
        branches = {'':1.0}
        ...
        for _val, _weight in branches.iteritems():
            ...
            #         mod_separator needed if 
            if parent != "":
                leaf_value = str(parent) + separator  
            else:
                leaf_value = "" 
            leaf_value += str(self.value)
            # 
            if _val != "":
    ...
    def add(self, attr_vals, parse_order, level, additional_data={}):
        ...
        if attr_vals.has_key(attr_name):        
            value = attr_vals[attr_name]
        else:
            value = None
